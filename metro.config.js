...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  try {
    ...
    function isInsidePolyfillDir(subdir, filePath) {
      const rel = path.relative(path.join(__dirname, 'polyfills', subdir), filePath);
      return rel === '' || (!rel.startsWith('..') && !path.isAbsolute(rel));
    }

    // inside resolveRequest, replace the original startsWith block with:
    if (
      isInsidePolyfillDir('native', context.originModulePath) ||
      isInsidePolyfillDir('web', context.originModulePath) ||
      isInsidePolyfillDir('shared', context.originModulePath)
    ) {
